...
for i in range(10):
    ...
    print(name1, name2)
    zz = cv2.imdecode(np.frombuffer(zfile.read(name1),np.uint8),1)
    xx = cv2.resize(zz, [96, 112], interpolation = cv2.INTER_AREA)
...
accuracy = eval_acc(0.3050, predicts)
print(accuracy)

def test_a_pair(name1, name2):
    # img1 = alignment(cv2.imdecode(np.frombuffer(zfile.read(name1),np.uint8),1),landmark[name1])
    # img2 = alignment(cv2.imdecode(np.frombuffer(zfile.read(name2),np.uint8),1),landmark[name2])
    ...
    img1 = cv2.resize(img1, [96, 112], interpolation = cv2.INTER_AREA)
    img2 = cv2.resize(img2, [96, 112], interpolation = cv2.INTER_AREA)
...
print("myfunc ", test_a_pair("Ann_Veneman/Ann_Veneman_0003.jpg", "Gary_Williams/Gary_Williams_0002.jpg"))
print("myfunc ", test_a_pair("Gary_Williams/Gary_Williams_0001.jpg", "Gary_Williams/Gary_Williams_0002.jpg"))
